...
//     // Password Validation(6 characters minnimum)
...
//         const userData = response.data.data.user; // Get only the user object
//         const token = response.data.data.token; // Make sure you're getting the token
    
//         // Save token to localStorage
//         localStorage.setItem("token", token);
//         console.log("User  Data before dispatch:", userData);
//         dispatch(setUser (userData)); // Dispatch only the user data
  ...
  const loginSubmitHandler = async (e: FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Explicitly ensure savedJobs exists on the user object
