...
const UpdateProfileDialog: React.FC<UpdateProfileDialogProps> = ({
  ...
  return (
    <Dialog
      ...
      onOpenChange={(newOpenState) => !loading && setOpen(newOpenState)}
    >
      <DialogContent className="w-full sm:max-w-[425px] max-h-[90vh] md:max-h-[85vh] overflow-y-auto p-4 md:p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Textarea
              id="bio"
              name="bio"
              className="w-full h-20 md:h-24 resize-none text-sm md:text-base"
              value={formData.bio}
              onChange={changeEventHandler}
              placeholder="Brief description about yourself"
            />
              ...
              <div className="mb-2">
                <div className="flex items-center gap-2 p-2 text-sm border rounded-md bg-blue-50">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1 text-xs md:text-sm text-blue-600 truncate hover:underline"
                  >
                    {user?.resumeOriginalName ||
                      getResumeFilename(formData.currentResume)}
                  </a>
                  <Button
                    ...
                    size="icon"
                    onClick={handleRemoveResume}
                    className="w-6 h-6 text-gray-500 hover:text-red-500 flex-shrink-0"
                    title="Remove resume"
                  >
                    <X className="w-3 h-3" />
                  </Button>
          ...
          {/* Action buttons */}
          <DialogFooter className="flex flex-col sm:flex-row justify-end gap-2 pt-4 mt-4 border-t">
            <Button
              type="button"
              variant="outline"
              onClick={() => setOpen(false)}
              disabled={loading}
              size="sm"
              className="w-full sm:w-24 text-sm py-2"
            >
              Cancel
            </Button>
            <Button
              ...
              disabled={loading}
              size="sm"
              className="w-full sm:w-24 bg-blue-600 hover:bg-blue-700 text-sm py-2"
            >
              {loading ? (
                <>
                  <Loader2 className="w-3 h-3 mr-2 animate-spin" />
